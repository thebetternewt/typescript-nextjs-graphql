...
function scanForFiles(globStr, globOptions = {}) {
    return sync(globStr, Object.assign({ absolute: true }, globOptions));
}
...
export function loadSchemaFiles(basePath, options = LoadSchemaFilesDefaultOptions) {
...
export function loadResolversFiles(basePath, options = LoadResolversFilesDefaultOptions) {
...
export async function loadSchemaFilesAsync(basePath, options = LoadSchemaFilesDefaultOptions) {
    const execOptions = Object.assign({}, LoadSchemaFilesDefaultOptions, options);
...
export async function loadResolversFilesAsync(basePath, options = LoadResolversFilesDefaultOptions) {
    const execOptions = Object.assign({}, LoadResolversFilesDefaultOptions, options);
    const relevantPaths = await scanForFilesAsync(buildGlob(basePath, execOptions.extensions, []), options.globOptions);
    const require$ = (path) => import(path);
    return Promise.all(relevantPaths.map(async (path) => {
