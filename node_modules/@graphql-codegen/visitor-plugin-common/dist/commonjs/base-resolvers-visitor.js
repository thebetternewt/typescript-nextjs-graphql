...
class BaseResolversVisitor extends base_visitor_1.BaseVisitor {
    constructor(rawConfig, additionalConfig, _schema, defaultScalars = scalars_1.DEFAULT_SCALARS) {
        super(rawConfig, {
            ...
            rootValueType: mappers_1.parseMapper(rawConfig.rootValueType || '{}'),
    ...
    get mappersImports() {
        ...
        if (this.config.defaultMapper && this.config.defaultMapper.isExternal) {
            ...
            const identifier = utils_1.stripMapperTypeInterpolation(this.config.defaultMapper.type);
...
export type IDirectiveResolvers${contextType} = ${name}<ContextType>;`
    ...
    FieldDefinition(node, key, parent) {
        ...
        return (parentName) => {
            ...
            return utils_1.indent(`${node.name}${this.config.avoidOptionals ? '' : '?'}: ${isSubscriptionType ? 'SubscriptionResolver' : 'Resolver'}<${mappedType}, ParentType, ContextType${hasArguments
